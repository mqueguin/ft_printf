...
/*   Updated: 2021/02/18 11:10:22 by mqueguin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_special_cases(int nbr, t_data *data, char *str)
{
	ft_treat_special_cases_int(nbr, data, str);
	if (nbr > 0 && data->plus == 0)
		data->len_variable = (int)ft_strlen(str);
	else if (nbr == 0 && data->b_dot == 0 && data->plus == 0)
		data->len_variable = 1;
	else if (nbr == 0 && data->b_dot == 1 && data->dot < 0
			&& data->dot_exist == 1 && data->plus == 0)
		data->len_variable = 1;
...
{
	if (data->space == 1 && nbr >= 0 && data->minus == 1)
...
void		ft_treat_int(int nbr, t_data *data)
{
	...
	ft_treat_int_second(nbr, data, space);
	if (data->minus == 0)
	{
		ft_treat_minus_first(nbr, data, space);
		ft_treat_minus_second(nbr, data, str, fill);
